...
|CI/CD Pipeline |`git clone && cd diag-agent && uv pip install . && uv run diag-agent create-batch ...`
...
|Quick local command |`cd diag-agent && uv run diag-agent create "diagram"` (no venv activation)
